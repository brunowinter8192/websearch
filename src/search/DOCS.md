...
**Purpose:** pydoll Chrome lifecycle — one shared, headed, backgrounded (macOS `open -g -n`) Chrome, one tab per engine for isolation, no JS fingerprint patches or UA override. `get_tab()`'s first-launch path blocks on a cross-process lock (`browser_lock.py`) scoped to the shared session profile, reaps any orphaned survivor of a crashed prior run (net 3), snapshots the real Chrome PID(s) it launched, then spawns a `death_pipe` watchdog (net 2 — no `cleanup_dir`, the session profile is persistent by design). As of the M1 no-focus-steal milestone (2026-09-15), `get_tab()` also spawns a PID-keyed focus-steal watchdog (`_spawn_focus_watchdog`/`_focus_steal_watchdog_by_pid`) — see Gotchas for why it is PID-keyed rather than name-keyed like the scraper package's sibling mechanism, and why its "known-good app to restore" anchor is captured by `get_tab()` itself before Chrome ever launches rather than by the watchdog's own first read. `kill_own_chrome()` is the deterministic own-run teardown (net 1 — graceful CDP close + PID-scoped psutil safety net + lock release, `close_browser()`'s own raise caught so net 1 can't itself skip the lock release), replacing profile-pattern `pkill` as the primary teardown path; `close_browser()` also cancels the focus watchdog task first, unconditionally, so the 40+ dev-script direct callers of `close_browser()` (see below) don't leak a live watchdog task.
...
**Calls out:** `pydoll` (Chrome, ChromiumOptions, BrowserProcessManager, TargetCommands); `psutil` (own-PID terminate/kill); `browser_lock` (acquire); `death_pipe` (spawn_watchdog); `open`/`pgrep`/`osascript` (macOS process control and frontmost-app/window control).
...
- **The focus-steal watchdog (`_focus_steal_watchdog_by_pid`, M1 milestone, 2026-09-15) is keyed on PID, not on app name, unlike the scraper package's `chromium_process.py::_focus_steal_watchdog`.** This module launches the user's actual `"Google Chrome"` bundle (bare name, `_open_background_process_creator`) rather than a dedicated resolved bundle — so a name-keyed watchdog cannot tell this run's own Chrome apart from the user's own separate, already-running Chrome (both report frontmost app name `"Google Chrome"`). Reusing the scraper lane's name-keyed watchdog as-is would re-background the user's OWN Chrome the moment they deliberately switched to it. `_owned_pids` (from `_record_own_pids`, already scoped to `user-data-dir=SESSION_DIR`) is the correct, already-existing fingerprint that distinguishes the two without ambiguity — `_get_frontmost_pid`/`_activate_pid` resolve/set the frontmost process by macOS `unix id` (System Events), never by name. Live-verified 2026-09-15: `set frontmost of (first process whose unix id is <pid>) to true` reactivates the exact target process even when another process shares its app name.
- **The watchdog's "known-good app to restore" anchor is captured by `get_tab()` itself (`anchor_pid = _get_frontmost_pid()`, right after `_reap_session_profile()`, before `Chrome(options)`/`_browser.start()`), and threaded into `_spawn_focus_watchdog`/`_focus_steal_watchdog_by_pid` as a parameter — the watchdog no longer derives its own initial reading.** A live probe (2026-09-15, instrumented with a traced `_activate_pid`) caught a real bug in an earlier version that read its own initial anchor via `_get_frontmost_pid()` inside the watchdog coroutine itself: since the watchdog only starts AFTER Chrome finishes launching — exactly the moment Chrome is likeliest to already be the frontmost window from its own launch — that self-read could capture an OWNED pid as the "anchor", which the reclaim condition then always rejects (`last_other_pid not in owned_pids` false), silently disabling reclaim until Chrome happened to cede focus on its own. Observed live: one run flickered for ~0.9s with zero `_activate_pid` calls (self-resolved, not reclaimed); another sustained a ~2.6s multi-engine steal streak with zero reclaims, both traced to a poisoned self-captured anchor. Fixed by capturing the anchor before Chrome can possibly steal anything, at the one point in `get_tab()` that structurally cannot race the launch. Regression-guarded by `test_focus_steal_watchdog_by_pid_reclaims_immediately_when_already_stolen_at_start` (`dev/tests/test_browser.py`) — an owned pid frontmost on the very FIRST watchdog loop iteration still reclaims immediately, given a valid externally-supplied anchor. Post-fix, 3 live verification runs each showed the flicker bounded to under ~1s (one `osascript`-driven `_activate_pid` call per steal, same `FOCUS_STEAL_POLL_INTERVAL_S=0.25s`-plus-subprocess-overhead bound the scraper lane's own watchdog already documents), with no unbounded/sustained steal observed after the fix — see `process-docs/browser_posture/` for the full probe transcripts.
- `close_browser()` cancels any live `_focus_watchdog_task` (`_cancel_focus_watchdog`) as the FIRST action, unconditionally, before touching `_browser.stop()` — not only in `kill_own_chrome()`'s teardown path. `close_browser()` is called directly by 40+ `dev/search_pipeline/*.py` probes that bypass `kill_own_chrome()` entirely (see this module's own `Called by` above); placing cancellation only in `kill_own_chrome()` would leak a live watchdog task (and its polling `osascript` calls) for every one of those direct callers.
