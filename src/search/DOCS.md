...
**Purpose:** pydoll Chrome lifecycle — one shared, headed, backgrounded (macOS `open -g -n`) Chrome, one tab per engine for isolation, no JS fingerprint patches or UA override. As of the 2026-09-15 self-launch milestone, `get_tab()` no longer calls pydoll's `Chrome.start()`/`.connect()` at all — both require an existing page-type CDP target and raise otherwise, which is incompatible with the `--no-startup-window` no-window launch this milestone needs. Instead `get_tab()` replicates the shape of `src/scraper/chromium_process.py`'s self-launch: construct `Chrome(options)` (still runs pydoll's own default-argument setup), call its own `_setup_user_dir()` directly (the only place `block_popups`/`block_notifications`/`browser_preferences` actually get written to disk — skipping `.start()` without this call would silently drop them), self-launch via the existing `_open_background_process_creator`, then wait for the `DevToolsActivePort` file exactly like the scrape lane. No tab is created during launch — `_get_valid_tab_id`'s CDP browser-level HTTP endpoint (`/json/version`) resolves lazily on the first real command, and every engine already creates its own tab via `new_tab()`, so there was never anything for a launch-time tab to be used for; there is no `_tab` module state anymore. `get_tab()`'s first-launch path blocks on a cross-process lock (`browser_lock.py`) scoped to the shared session profile, reaps any orphaned survivor of a crashed prior run (net 3), snapshots the real Chrome PID(s) it launched, then spawns a `death_pipe` watchdog (net 2 — no `cleanup_dir`, the session profile is persistent by design). As of the M1 no-focus-steal milestone (2026-09-15), `get_tab()` also spawns a PID-keyed focus-steal watchdog (`_spawn_focus_watchdog`/`_focus_steal_watchdog_by_pid`) — see Gotchas for why it is PID-keyed rather than name-keyed like the scraper package's sibling mechanism, and why its "known-good app to restore" anchor is captured by `get_tab()` itself before Chrome ever launches rather than by the watchdog's own first read. `kill_own_chrome()` is the deterministic own-run teardown (net 1 — graceful CDP close + PID-scoped psutil safety net + lock release, `close_browser()`'s own raise caught so net 1 can't itself skip the lock release), replacing profile-pattern `pkill` as the primary teardown path; `close_browser()` also cancels the focus watchdog task first, unconditionally, so the 40+ dev-script direct callers of `close_browser()` (see below) don't leak a live watchdog task.
...
- **The focus-steal watchdog (`_focus_steal_watchdog_by_pid`) is keyed on PID, not on app name, unlike the scraper package's `chromium_process.py::_focus_steal_watchdog`.** This module launches the user's actual `"Google Chrome"` bundle (bare name, `_open_background_process_creator`), not a dedicated resolved bundle — a name-keyed watchdog cannot tell this run's own Chrome apart from the user's own separate, already-running Chrome (both report the same frontmost app name), and would re-background the user's OWN Chrome the moment they deliberately switched to it. `_owned_pids` (from `_record_own_pids`, already scoped to `user-data-dir=SESSION_DIR`) is the fingerprint that distinguishes the two without ambiguity. See `process-docs/browser_posture/` for the reasoning and live verification.
