...
**Purpose:** Cross-engine URL grouping — groups results by URL and keeps one entry per engine that returned it (same-engine duplicates still collapse to one, lower position wins), each entry annotated with `engine_positions` for every engine that offered that URL; returns a per-engine pool dict sorted by native position. As of the M2 dedup-inversion milestone (2026-09-19) a URL is no longer reassigned to a single winning engine — every engine that returned it keeps its own entry. The fresh `SearchResult` built per entry names fields explicitly, so a new field (`date`, `pdf_url`) must be added here explicitly too or it silently drops.
...
**Purpose:** pydoll Chrome lifecycle — one shared, headed, backgrounded Chrome self-launched via a dynamically resolved bundle, one tab per engine.
**Reads:** nothing (singleton browser on first access).
**Writes:** Chrome session dir under the user-data-dir; the cross-process lock file + JSON sidecar.
...
- **`get_tab()`'s stale-`DevToolsActivePort`-file race is specific to this module, not shared with the scrape lane.** `src/scraper/chromium_process.py` uses a fresh `tempfile.mkdtemp()` per scrape, so a leftover `DevToolsActivePort` file from a prior run is structurally impossible there. This module reuses one persistent `SESSION_DIR` across every run, so a crashed prior Chrome can leave a stale file behind; `_clear_stale_devtools_port()` deletes it before every launch so the readiness wait can only ever read a freshly-written port.
...
- Post-dedup pool cap is a fixed 10 per engine (`POOL_CAP` in `search_web.py`), independent of what any other engine returned. Was formerly anchored to Google's own pool size (falling back to 10 only when Google returned zero) — dropped 2026-09 once Google's own result count stopped being a reliable 8-11 and started being 0 in the overwhelming majority of queries, which had been collapsing every engine's pool to Google's occasional 1 or 2. As of the M2 dedup-inversion milestone (below), a URL shared by several engines now legitimately consumes a cap slot in EACH of those engines' own top-10 independently — deliberate, since every engine's pool is now a function of only that engine's own results, with no cross-engine coupling left in the pipeline. See `process-docs/search_pipeline/` for the measurement and the ordering reasoning.
- **As of the M2 dedup-inversion milestone (2026-09-19), `build_engine_pools` no longer reassigns a shared URL to a single winning engine — every engine that returned it keeps its own pool entry, annotated with `engine_positions` for every engine that offered it.** The prior winner-take-all dedup measurably cost `duckduckgo`/`brave`/`bing`/`startpage`/`yandex` 7-20% of everything they returned, for a cross-engine-uniqueness benefit that only pays off when several engines get drilled per query — real query-log data showed that happens in roughly 1 of 6 drilled searches, the rest drill exactly one engine and simply eat the loss. `engine_positions` is now persisted through `cache_write` (previously silently dropped during serialization) but is NOT rendered in `format_engine_pool`'s text output — computed and available, not acted on today.
