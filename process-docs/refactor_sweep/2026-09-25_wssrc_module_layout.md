...
## Follow-up after the four-eyes review (2026-09-25)

The review found no behaviour defect. Fixed afterwards:
- `pipe_scraper.py`: the script entry `run_cli` is now the one orchestrator (calls `_parse_args`, `_read_url_file`, then `asyncio.run(scrape_urls_workflow(...))`); `scrape_urls_workflow` and its helpers are ordinary functions below it and reachable from it. This replaces the "inherent exception" stated above; the stepdown count is 0.
- `death_pipe.py`: `spawn_watchdog` moved into its own library module `src/watchdog_spawn.py` (one function, no orchestrator). `death_pipe.py` is now only the detached script (orchestrator `_watchdog_main`) plus `terminate_then_kill`, both reachable. Tests that patched `death_pipe.spawn_watchdog` patch `watchdog_spawn.spawn_watchdog`; the spawner now starts `death_pipe.py` by the path next to itself.
- Six constants that one module imported from another moved to `src/config.py`: `TOTAL_SCRAPE_BUDGET_S`, `SCRAPE_LOG_PATH` (was `scrape_logger.DEFAULT_LOG_PATH`, also read by `index_scrapes`; the other two `DEFAULT_LOG_PATH` names are separate files and stay), `MONOSANS_URL`, `MAX_SNIPPET_LEN`, the three riding values (now `RIDING_PAGE_TIMEOUT_MS`, `RIDING_STALL_TIMEOUT_S`, `RIDING_POOL_REFRESH_INTERVAL_S`, prefixed because `STALL_TIMEOUT_S` in `proxy_pool/loop.py` is a different constant), and `PROXY_POOL_BUFFER_SIZE` / `PROXY_POOL_CONCURRENCY`. No constant is left that one src module imports from another module of its own group except the dedicated `seed_feeders_constants.py` / `pipe_scraper_constants.py` / per-platform `config.py` modules named above.
