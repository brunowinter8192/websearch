...
- **Since the self-launch milestone, `get_tab()` skips the two pieces of pydoll's `Chrome.start()` that only fire through `.start()`/`.connect()`: proxy-credential configuration and the `--user-agent=` override (both browser-level and per-worker).** `build_options()` sets neither today, so this is as of 2026-09-24 a no-op gap, not an observed regression — but it means proxy support or a UA override cannot just be added to `build_options()` the way `webrtc_leak_protection`/`BACKGROUNDING_FLAGS` were; the equivalent pydoll-internal calls (`_configure_proxy`, `_apply_user_agent_override`, `_setup_worker_user_agent_override`) would need to be called explicitly from `get_tab()` too. Flagged here once so it isn't rediscovered as a mystery later.
