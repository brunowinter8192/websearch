...
# Phase 4 control-flow triage: dev/search_pipeline (2026-09-24)

Step 1 only: classification, no code changed. Scope: every `except` handler under `dev/search_pipeline/` (all subfolders). The scan (`exscan.py` heuristic, copied to a private directory) finds 118 handlers: 90 PRODUCES-OUTPUT, 11 SWALLOW-PASS, 8 LOG-ONLY, 9 TRIPWIRE. The 9 tripwires are not classified: `_rerank_probe_smoke_gpu.py` (2), `bee_probes/_acquire_probe_instrument.py:64` and `_branch_probe_instrument.py:53` (BaseException re-raise), `browser_probes/altcha_trigger_probe.py:145,172`, `ranking_eval/value_eval_probe.py:148`, `scholar_http_probe.py:48,51`. The remaining 109 are in the table.

## Classes

A status/report (exception becomes a recorded failure fact), B fallback (same kind of output produced a second way: default value, raw instead of parsed, empty list that looks like a genuine empty result), C silent swallow of an error that affects results, D best-effort teardown, E input-shape handling where the except branch is a real outcome. Verdicts: allowed, B-remove, B-keep, B-keep-needs-logging, remove.

Counts: A 44, B 41, C 2, D 12, E 10 (109). Verdicts: allowed 66, B-remove 37, B-keep-needs-logging 4, remove (class C) 2.

## Evidence gathered (measured 2026-09-24)

- `urlparse` handlers: every URL found in this area's `md/`, `txt/`, `jsonl/`, `runs/`, `inspections/md/` plus the main checkout `src/logs/query_log.jsonl` (9208 distinct URLs) was run through `urlparse(url.strip())`: 0 raised.
- `_lib/parse._repr_unquote`: 4142 repr-quoted fields (per-engine snippet lines and `display:` lines) in the four `md/pipeline_smoke_*.md` reports: `ast.literal_eval` failed on 0, so the quote-stripping fallback never fired on real data.
- JSON-parse handlers in the browser probes and inspection scripts: the six production engines lost the identical `_parse_results` handler in an earlier Phase 4 pass with the evidence 0 `ERROR_PARSE` in 4566 logged engine records and the parsed value being the script's own `JSON.stringify` output (see the `refactor_sweep` area). That entry explicitly left the separate `_diagnose` handler untouched and unexamined; here it is examined and has no observation either. The `_extract_value` helper (KeyError/TypeError to None) is still present in `src/search/engines/*.py`, was kept by that pass, and its None is consumed by explicit `if not value` guards: treated as input shape.
- GPU method failures: 48 `runs/*/*_methods*.json` files were scanned for empty method arrays. Only two pairs (`value_eval_20260522_015113`, both `books`) hold empty arrays, and they come from `google_count 0` with `filtered_pool_size 0` (empty pool), not from a failed call. No artifact shows a failed reranker, SPLADE or generator call.
- Retry loops: `jsonl/pooling_probe_20260521_215844.queries.jsonl`, 20 queries, has exactly one non-skipped query with an empty ranking (`gradient descent optimization ...`: pool 84, C3 11 URLs, C4 embedding-cosine 0 URLs). That is the one observation of an API failure surviving the retry; the retry attempts and the final failure went only to stderr. The code comment that 500s from the llama-server are transient (moved to this file in Phase 2) is the only other statement.
- `query_log.jsonl`: 579 of 579 lines parse as JSON.
- `books_probe` and `docs_probe` reports show `0 errors` for every engine, so the exception path of the engine loop was never observed there either; those handlers still count as A because they record the failure.
- `cdp_query_selector`: no process-docs entry documents a `DOM.querySelector` exception. CDP reports a non-match with nodeId 0, so the not-found case does not need the handler.

## Full table

| file:line | function | scan | class | verdict | evidence |
|---|---|---|---|---|---|
| `00_single_query.py:136` | `_print_parsed_results` | SWALLOW-PASS | C | remove | json.loads failure passes silently and 'Parsed results: 0' looks like a genuine empty parse; no observation (script currently fails at import) |
| `00_single_query.py:149` | `_close_tab_and_browser` | SWALLOW-PASS | D | allowed | tab.close() teardown |
| `01_google_smoke.py:69` | `run_query` | PRODUCES-OUTPUT | A | allowed | exception recorded as status ERROR plus error text in the record |
| `02_burst_smoke.py:112` | `run_batch` | PRODUCES-OUTPUT | A | allowed | timeout and exception recorded as ERROR records with note |
| `02_burst_smoke.py:165` | `_domain` | PRODUCES-OUTPUT | B | B-remove | 0 of 9208 URLs found in this area's md/txt/jsonl/runs data and the main checkout query_log raise in urlparse; default value hides a malformed URL as an empty domain |
| `02_burst_smoke.py:89` | `run_batch` | PRODUCES-OUTPUT | A | allowed | timeout and exception recorded as ERROR records with note |
| `04_ddg_smoke.py:69` | `run_query` | PRODUCES-OUTPUT | A | allowed | exception recorded as status ERROR plus error text in the record |
| `08_scholar_smoke.py:69` | `run_query` | PRODUCES-OUTPUT | A | allowed | exception recorded as status ERROR plus error text in the record |
| `09_openalex_smoke.py:65` | `run_query` | PRODUCES-OUTPUT | A | allowed | exception recorded as status ERROR plus error text in the record |
| `12_max_results_probe.py:102` | `probe_single` | PRODUCES-OUTPUT | A | allowed | exception recorded as status ERROR plus error text in the record |
| `13_free_word_probe.py:251` | `_domain` | PRODUCES-OUTPUT | B | B-remove | 0 of 9208 URLs found in this area's md/txt/jsonl/runs data and the main checkout query_log raise in urlparse; default value hides a malformed URL as an empty domain |
| `13_free_word_probe.py:93` | `run_probe` | PRODUCES-OUTPUT | C | remove | exception only printed to stderr; engine results are silently missing from the report, indistinguishable from an empty engine; books/docs probes count errors, this one does not; 0 observed |
| `13_timing_ablation.py:146` | `engine_timed` | PRODUCES-OUTPUT | A | allowed | exception recorded as status ERROR plus error text in the record |
| `24_pydoll_teardown_verify.py:78` | `_count_renderers` | PRODUCES-OUTPUT | E | allowed | pgrep -c prints no integer when nothing matches or reports 0 for headless Chrome on macOS; 0 is the real answer |
| `24_pydoll_teardown_verify.py:131` | `_run_hung_op` | PRODUCES-OUTPUT | A | allowed | timeout and unexpected exception are written into the report lines and decide PASS/FAIL |
| `24_pydoll_teardown_verify.py:133` | `_run_hung_op` | PRODUCES-OUTPUT | A | allowed | timeout and unexpected exception are written into the report lines and decide PASS/FAIL |
| `24_pydoll_teardown_verify.py:163` | `_test_normal_tab_cleanup` | PRODUCES-OUTPUT | A | allowed | timeout and unexpected exception are written into the report lines and decide PASS/FAIL |
| `24_pydoll_teardown_verify.py:165` | `_test_normal_tab_cleanup` | PRODUCES-OUTPUT | A | allowed | timeout and unexpected exception are written into the report lines and decide PASS/FAIL |
| `24_pydoll_teardown_verify.py:255` | `_watchdog_wrapped` | SWALLOW-PASS | A | allowed | TimeoutError is the expected outcome (all_timed_out stays True); other exceptions are reported and flip the flag |
| `24_pydoll_teardown_verify.py:257` | `_watchdog_wrapped` | PRODUCES-OUTPUT | A | allowed | TimeoutError is the expected outcome (all_timed_out stays True); other exceptions are reported and flip the flag |
| `_capture_sorry.py:175` | `stop_browser` | SWALLOW-PASS | D | allowed | browser.stop() teardown |
| `_lib/parse.py:19` | `_repr_unquote` | PRODUCES-OUTPUT | B | B-remove | 0 of 4142 repr-quoted fields in the four real pipeline_smoke reports need the fallback (literal_eval always succeeds) |
| `browser_probes/25_startpage_probe.py:162` | `_kill_tab` | LOG-ONLY | D | allowed | close_target failure logged via logging.warning; teardown |
| `browser_probes/25_startpage_probe.py:178` | `_extract_value` | PRODUCES-OUTPUT | E | allowed | CDP omits result.value for undefined/null; identical helper kept in src/search/engines/*; None is a real outcome consumed by the callers' guards |
| `browser_probes/25_startpage_probe.py:213` | `_parse_results` | PRODUCES-OUTPUT | B | B-remove | src precedent: same handler removed from the six production engines (0 ERROR_PARSE in 4566 logged records, value is the probe's own JSON.stringify output); a parse failure is shown as an empty result |
| `browser_probes/25_startpage_probe.py:256` | `run_query` | PRODUCES-OUTPUT | A | allowed | exception recorded as status ERROR plus error text in the record |
| `browser_probes/25_startpage_probe.py:225` | `_diagnose_empty` | SWALLOW-PASS | B | B-remove | default diag looks like 'no block marker'; caller's outer except already records status ERROR; no observation of a malformed diagnose payload |
| `browser_probes/26_brave_probe.py:153` | `_kill_tab` | LOG-ONLY | D | allowed | close_target failure logged via logging.warning; teardown |
| `browser_probes/26_brave_probe.py:169` | `_extract_value` | PRODUCES-OUTPUT | E | allowed | CDP omits result.value for undefined/null; identical helper kept in src/search/engines/*; None is a real outcome consumed by the callers' guards |
| `browser_probes/26_brave_probe.py:190` | `_parse_results` | PRODUCES-OUTPUT | B | B-remove | src precedent: same handler removed from the six production engines (0 ERROR_PARSE in 4566 logged records, value is the probe's own JSON.stringify output); a parse failure is shown as an empty result |
| `browser_probes/26_brave_probe.py:231` | `run_query` | PRODUCES-OUTPUT | A | allowed | exception recorded as status ERROR plus error text in the record |
| `browser_probes/26_brave_probe.py:202` | `_diagnose` | SWALLOW-PASS | B | B-remove | default diag looks like 'no block marker'; caller's outer except already records status ERROR; no observation of a malformed diagnose payload |
| `browser_probes/27_brave_headed_lane_probe.py:151` | `_extract_value` | PRODUCES-OUTPUT | E | allowed | CDP omits result.value for undefined/null; identical helper kept in src/search/engines/*; None is a real outcome consumed by the callers' guards |
| `browser_probes/27_brave_headed_lane_probe.py:172` | `_parse_results` | PRODUCES-OUTPUT | B | B-remove | src precedent: same handler removed from the six production engines (0 ERROR_PARSE in 4566 logged records, value is the probe's own JSON.stringify output); a parse failure is shown as an empty result |
| `browser_probes/27_brave_headed_lane_probe.py:213` | `run_query` | PRODUCES-OUTPUT | A | allowed | exception recorded as status ERROR plus error text in the record |
| `browser_probes/27_brave_headed_lane_probe.py:142` | `_stop_headed_background_browser` | LOG-ONLY | D | allowed | browser.stop() failure logged, pkill safety net follows |
| `browser_probes/27_brave_headed_lane_probe.py:184` | `_diagnose` | SWALLOW-PASS | B | B-remove | default diag looks like 'no block marker'; caller's outer except already records status ERROR; no observation of a malformed diagnose payload |
| `browser_probes/28_bing_probe.py:154` | `_kill_tab` | LOG-ONLY | D | allowed | close_target failure logged via logging.warning; teardown |
| `browser_probes/28_bing_probe.py:170` | `_extract_value` | PRODUCES-OUTPUT | E | allowed | CDP omits result.value for undefined/null; identical helper kept in src/search/engines/*; None is a real outcome consumed by the callers' guards |
| `browser_probes/28_bing_probe.py:186` | `_clean_url` | PRODUCES-OUTPUT | B | B-remove | raw href returned instead of decoded URL; same fallback removed from src bing._clean_url (0 bing.com/ck/a URLs in the query log, 0 in 400 cache files) |
| `browser_probes/28_bing_probe.py:207` | `_parse_results` | PRODUCES-OUTPUT | B | B-remove | src precedent: same handler removed from the six production engines (0 ERROR_PARSE in 4566 logged records, value is the probe's own JSON.stringify output); a parse failure is shown as an empty result |
| `browser_probes/28_bing_probe.py:250` | `run_query` | PRODUCES-OUTPUT | A | allowed | exception recorded as status ERROR plus error text in the record |
| `browser_probes/28_bing_probe.py:225` | `_diagnose` | SWALLOW-PASS | B | B-remove | default diag looks like 'no block marker'; caller's outer except already records status ERROR; no observation of a malformed diagnose payload |
| `browser_probes/29_yandex_probe.py:153` | `_kill_tab` | LOG-ONLY | D | allowed | close_target failure logged via logging.warning; teardown |
| `browser_probes/29_yandex_probe.py:169` | `_extract_value` | PRODUCES-OUTPUT | E | allowed | CDP omits result.value for undefined/null; identical helper kept in src/search/engines/*; None is a real outcome consumed by the callers' guards |
| `browser_probes/29_yandex_probe.py:190` | `_parse_results` | PRODUCES-OUTPUT | B | B-remove | src precedent: same handler removed from the six production engines (0 ERROR_PARSE in 4566 logged records, value is the probe's own JSON.stringify output); a parse failure is shown as an empty result |
| `browser_probes/29_yandex_probe.py:227` | `run_query` | PRODUCES-OUTPUT | A | allowed | exception recorded as status ERROR plus error text in the record |
| `browser_probes/29_yandex_probe.py:202` | `_diagnose` | SWALLOW-PASS | B | B-remove | default diag looks like 'no block marker'; caller's outer except already records status ERROR; no observation of a malformed diagnose payload |
| `browser_probes/31_date_availability_probe.py:109` | `_dump_date_evidence` | PRODUCES-OUTPUT | B | B-remove | src precedent: same handler removed from the six production engines (0 ERROR_PARSE in 4566 logged records, value is the probe's own JSON.stringify output); a parse failure is shown as an empty result |
| `browser_probes/31_date_availability_probe.py:130` | `run_engine_query` | PRODUCES-OUTPUT | A | allowed | exception recorded as status ERROR plus error text in the record |
| `browser_probes/_altcha_trigger_probe_cdp.py:15` | `cdp_query_selector` | PRODUCES-OUTPUT | B | B-remove | CDP protocol errors are turned into 'node not found' so the shadow-root -> light-DOM -> host fallback chain continues; DOM.querySelector reports no match with nodeId 0, so no-match does not need the exception path; no observed DOM.querySelector exception in process-docs |
| `browser_probes/_date_availability_probe_browser.py:102` | `_kill_tab` | LOG-ONLY | D | allowed | close_target failure logged via logging.warning; teardown |
| `browser_probes/_date_availability_probe_browser.py:118` | `_extract_value` | PRODUCES-OUTPUT | E | allowed | CDP omits result.value for undefined/null; identical helper kept in src/search/engines/*; None is a real outcome consumed by the callers' guards |
| `browser_probes/_date_availability_probe_browser.py:150` | `_generic_diagnose` | PRODUCES-OUTPUT | B | B-remove | default diag looks like 'no block marker'; caller's outer except already records status ERROR; no observation of a malformed diagnose payload |
| `browser_probes/altcha_trigger_probe.py:155` | `_close_probe_session` | SWALLOW-PASS | D | allowed | session teardown; failures logged, CancelledError from cancelled watchdog is expected |
| `browser_probes/altcha_trigger_probe.py:159` | `_close_probe_session` | LOG-ONLY | D | allowed | session teardown; failures logged, CancelledError from cancelled watchdog is expected |
| `browser_probes/altcha_trigger_probe.py:163` | `_close_probe_session` | LOG-ONLY | D | allowed | session teardown; failures logged, CancelledError from cancelled watchdog is expected |
| `browser_probes/altcha_trigger_probe.py:183` | `_navigate` | PRODUCES-OUTPUT | A | allowed | navigation error string returned and stored as TriggerResult.error |
| `browser_probes/altcha_trigger_probe.py:207` | `_confirm_widget_ready` | PRODUCES-OUTPUT | A | allowed | element_found=False recorded in the result and the wait failure is logged |
| `browser_probes/altcha_trigger_probe.py:198` | `_wait_for_event` | PRODUCES-OUTPUT | E | allowed | timeout without an event is a real outcome; the caller inspects the collected events |
| `domain_probes/19_books_probe.py:288` | `_domain` | PRODUCES-OUTPUT | B | B-remove | 0 of 9208 URLs found in this area's md/txt/jsonl/runs data and the main checkout query_log raise in urlparse; default value hides a malformed URL as an empty domain |
| `domain_probes/19_books_probe.py:90` | `run_probe` | PRODUCES-OUTPUT | A | allowed | exception printed and counted in run_stats['errors'], shown in the report table (0 errors observed in the two reports) |
| `domain_probes/20_docs_probe.py:79` | `run_probe` | SWALLOW-PASS | D | allowed | close_browser() teardown |
| `domain_probes/20_docs_probe.py:65` | `run_probe` | PRODUCES-OUTPUT | A | allowed | exception printed and counted in run_stats['errors'], shown in the report table |
| `domain_probes/_docs_probe_report.py:340` | `_match_heuristics` | PRODUCES-OUTPUT | B | B-remove | 0 of 9208 URLs found in this area's md/txt/jsonl/runs data and the main checkout query_log raise in urlparse; default value hides a malformed URL as an empty domain; an exception makes the URL count as 'matches none' |
| `domain_probes/_docs_probe_report.py:348` | `_domain` | PRODUCES-OUTPUT | B | B-remove | 0 of 9208 URLs found in this area's md/txt/jsonl/runs data and the main checkout query_log raise in urlparse; default value hides a malformed URL as an empty domain |
| `empty_classify_se.py:117` | `_probe_stackoverflow` | PRODUCES-OUTPUT | A | allowed | request error stored in record notes, classification stays UNKNOWN |
| `empty_classify_se.py:134` | `_probe_cross_site` | PRODUCES-OUTPUT | A | allowed | request error appended to record notes |
| `google_selector_probe.py:136` | `read_counts` | PRODUCES-OUTPUT | B | B-remove | src precedent: same handler removed from the six production engines (0 ERROR_PARSE in 4566 logged records, value is the probe's own JSON.stringify output); a parse failure is shown as an empty result |
| `google_selector_probe.py:147` | `read_structure` | PRODUCES-OUTPUT | B | B-remove | src precedent: same handler removed from the six production engines (0 ERROR_PARSE in 4566 logged records, value is the probe's own JSON.stringify output); a parse failure is shown as an empty result |
| `inspections/inspect_engine_dom.py:148` | `_extract_value` | PRODUCES-OUTPUT | E | allowed | CDP omits result.value for undefined/null; identical helper kept in src/search/engines/*; None is a real outcome consumed by the callers' guards |
| `inspections/inspect_engine_dom.py:159` | `run_js_dict` | PRODUCES-OUTPUT | B | B-remove | src precedent: same handler removed from the six production engines (0 ERROR_PARSE in 4566 logged records, value is the probe's own JSON.stringify output); a parse failure is shown as an empty result |
| `inspections/inspect_engine_dom.py:168` | `run_js_int` | PRODUCES-OUTPUT | B | B-remove | int() failure returns 0, i.e. a genuine-looking zero count; None already handled by 'val or 0'; no observation |
| `no_google_burst_smoke.py:142` | `_run_engine` | PRODUCES-OUTPUT | A | allowed | each exception mapped to an explicit status value (TIMEOUT_WATCHDOG, TIMEOUT_HTTPX, ERROR_BROWSER, ERROR_HTTP, ERROR) |
| `no_google_burst_smoke.py:145` | `_run_engine` | PRODUCES-OUTPUT | A | allowed | each exception mapped to an explicit status value (TIMEOUT_WATCHDOG, TIMEOUT_HTTPX, ERROR_BROWSER, ERROR_HTTP, ERROR) |
| `no_google_burst_smoke.py:148` | `_run_engine` | PRODUCES-OUTPUT | A | allowed | each exception mapped to an explicit status value (TIMEOUT_WATCHDOG, TIMEOUT_HTTPX, ERROR_BROWSER, ERROR_HTTP, ERROR) |
| `no_google_burst_smoke.py:151` | `_run_engine` | PRODUCES-OUTPUT | A | allowed | each exception mapped to an explicit status value (TIMEOUT_WATCHDOG, TIMEOUT_HTTPX, ERROR_BROWSER, ERROR_HTTP, ERROR) |
| `no_google_burst_smoke.py:154` | `_run_engine` | PRODUCES-OUTPUT | A | allowed | each exception mapped to an explicit status value (TIMEOUT_WATCHDOG, TIMEOUT_HTTPX, ERROR_BROWSER, ERROR_HTTP, ERROR) |
| `pdf_probes/15_citation_pdf_followup.py:191` | `_hop1_extract` | PRODUCES-OUTPUT | A | allowed | exception mapped to an outcome string (TIMEOUT, CONN_ERROR:<type>, ERROR:<type>) that enters the report |
| `pdf_probes/15_citation_pdf_followup.py:193` | `_hop1_extract` | PRODUCES-OUTPUT | A | allowed | exception mapped to an outcome string (TIMEOUT, CONN_ERROR:<type>, ERROR:<type>) that enters the report |
| `pdf_probes/15_citation_pdf_followup.py:195` | `_hop1_extract` | PRODUCES-OUTPUT | A | allowed | exception mapped to an outcome string (TIMEOUT, CONN_ERROR:<type>, ERROR:<type>) that enters the report |
| `pdf_probes/15_citation_pdf_followup.py:234` | `_hop2_classify` | PRODUCES-OUTPUT | A | allowed | exception mapped to an outcome string (TIMEOUT, CONN_ERROR:<type>, ERROR:<type>) that enters the report |
| `pdf_probes/15_citation_pdf_followup.py:236` | `_hop2_classify` | PRODUCES-OUTPUT | A | allowed | exception mapped to an outcome string (TIMEOUT, CONN_ERROR:<type>, ERROR:<type>) that enters the report |
| `pdf_probes/15_citation_pdf_followup.py:238` | `_hop2_classify` | PRODUCES-OUTPUT | A | allowed | exception mapped to an outcome string (TIMEOUT, CONN_ERROR:<type>, ERROR:<type>) that enters the report |
| `pdf_probes/15_citation_pdf_followup.py:247` | `_base_domain` | PRODUCES-OUTPUT | B | B-remove | 0 of 9208 URLs found in this area's md/txt/jsonl/runs data and the main checkout query_log raise in urlparse; default value hides a malformed URL as an empty domain |
| `pdf_probes/16_search_to_pdf_probe.py:204` | `_get_pdf_and_save` | PRODUCES-OUTPUT | A | allowed | exception mapped to an outcome string (TIMEOUT, CONN_ERROR:<type>) that enters the report |
| `pdf_probes/16_search_to_pdf_probe.py:206` | `_get_pdf_and_save` | PRODUCES-OUTPUT | A | allowed | exception mapped to an outcome string (TIMEOUT, CONN_ERROR:<type>) that enters the report |
| `pdf_probes/16_search_to_pdf_probe.py:208` | `_get_pdf_and_save` | PRODUCES-OUTPUT | A | allowed | exception mapped to an outcome string (TIMEOUT, CONN_ERROR:<type>) that enters the report |
| `pdf_probes/16_search_to_pdf_probe.py:227` | `_extract_citation_pdf_url` | PRODUCES-OUTPUT | B | B-remove | any fetch exception becomes None, which the caller records as NO_PDF_LINK, mislabelling a network error as 'page has no citation link'; sibling _get_pdf_and_save records CONN_ERROR instead; no observation |
| `pdf_probes/16_search_to_pdf_probe.py:263` | `_base_domain` | PRODUCES-OUTPUT | B | B-remove | 0 of 9208 URLs found in this area's md/txt/jsonl/runs data and the main checkout query_log raise in urlparse; default value hides a malformed URL as an empty domain |
| `pdf_probes/_download_classify_probe_classify.py:88` | `_classify_url` | PRODUCES-OUTPUT | A | allowed | exception mapped to outcome CONNECTION_ERROR plus page_title with the error text |
| `pdf_probes/_download_classify_probe_classify.py:90` | `_classify_url` | PRODUCES-OUTPUT | A | allowed | exception mapped to outcome CONNECTION_ERROR plus page_title with the error text |
| `pdf_probes/_download_classify_probe_classify.py:93` | `_classify_url` | PRODUCES-OUTPUT | A | allowed | exception mapped to outcome CONNECTION_ERROR plus page_title with the error text |
| `pdf_probes/_download_classify_probe_classify.py:96` | `_classify_url` | PRODUCES-OUTPUT | A | allowed | exception mapped to outcome CONNECTION_ERROR plus page_title with the error text |
| `pdf_probes/_download_classify_probe_classify.py:156` | `_classify_html_body` | PRODUCES-OUTPUT | B | B-remove | bytes.decode(errors='replace') cannot raise; the handler is unreachable |
| `pdf_probes/_download_classify_probe_pool.py:57` | `_has_real_path` | PRODUCES-OUTPUT | B | B-remove | 0 of 9208 URLs found in this area's md/txt/jsonl/runs data and the main checkout query_log raise in urlparse; default value hides a malformed URL as an empty domain |
| `pdf_probes/_download_classify_probe_pool.py:78` | `_base_domain` | PRODUCES-OUTPUT | B | B-remove | 0 of 9208 URLs found in this area's md/txt/jsonl/runs data and the main checkout query_log raise in urlparse; default value hides a malformed URL as an empty domain |
| `pydoll_fingerprint_probe.py:84` | `_extract_value` | PRODUCES-OUTPUT | E | allowed | CDP omits result.value for undefined/null; identical helper kept in src/search/engines/*; None is a real outcome consumed by the callers' guards |
| `ranking_eval/_stage3_method_run_v3_gpu.py:58` | `_rerank_pool` | PRODUCES-OUTPUT | B | B-remove | 0 GPU-failure artifacts in 48 methods JSON files (only empties come from empty pools with google_count 0); failure is only printed to stderr and stored as an empty ranking |
| `ranking_eval/_stage3_method_run_v3_gpu.py:108` | `_apply_m9` | PRODUCES-OUTPUT | B | B-remove | 0 GPU-failure artifacts in 48 methods JSON files (only empties come from empty pools with google_count 0); failure is only printed to stderr and stored as an empty ranking |
| `ranking_eval/_stage3_method_run_v3_gpu.py:163` | `_call_generator` | PRODUCES-OUTPUT | B | B-remove | 0 GPU-failure artifacts in 48 methods JSON files (only empties come from empty pools with google_count 0); failure is only printed to stderr and stored as an empty ranking |
| `ranking_eval/pooling_probe.py:125` | `_safe_rerank` | PRODUCES-OUTPUT | B | B-keep-needs-logging | retry on API error is observed: pooling run 20260521_215844 query 'gradient descent optimization ...' has c4 empty with pool 84 (c3 11) and the code comment records 500s as transient; second attempt and final failure appear only on stderr, the artifact shows an empty ranking |
| `ranking_eval/pooling_probe.py:140` | `_safe_embed` | PRODUCES-OUTPUT | B | B-keep-needs-logging | retry on API error is observed: pooling run 20260521_215844 query 'gradient descent optimization ...' has c4 empty with pool 84 (c3 11); attempts logged to stderr only, the artifact shows an empty ranking |
| `ranking_eval/single_query_pool_dump.py:146` | `_ce_top_scored` | PRODUCES-OUTPUT | B | B-keep-needs-logging | same retry shape as pooling_probe._safe_rerank (observed transient API 500s, see pooling run 20260521_215844); attempts logged to stderr only, final failure looks like an empty config in the report |
| `ranking_eval/single_query_pool_dump.py:164` | `_embed_top_scored` | PRODUCES-OUTPUT | B | B-keep-needs-logging | same retry shape as pooling_probe._safe_embed (observed embedding failure, see pooling run 20260521_215844); attempts logged to stderr only, final failure looks like an empty config in the report |
| `ranking_eval/stage3_method_run.py:132` | `_apply_c3` | PRODUCES-OUTPUT | B | B-remove | 0 GPU-failure artifacts in 48 methods JSON files (only empties come from empty pools with google_count 0); failure is only printed to stderr and stored as an empty ranking |
| `ranking_eval/value_eval_probe.py:168` | `_url_domain` | PRODUCES-OUTPUT | B | B-remove | 0 of 9208 URLs found in this area's md/txt/jsonl/runs data and the main checkout query_log raise in urlparse; default value hides a malformed URL as an empty domain |
| `ranking_eval/value_eval_probe.py:238` | `_apply_c3` | PRODUCES-OUTPUT | B | B-remove | 0 GPU-failure artifacts in 48 methods JSON files (only empties come from empty pools with google_count 0); failure is only printed to stderr and stored as an empty ranking |
| `with_google_decoupling_smoke.py:84` | `_read_last_log_entry` | PRODUCES-OUTPUT | B | B-remove | query_log.jsonl: 579 of 579 lines parse; IndexError is unreachable behind 'if lines'; {} then reads as a failed invariant instead of an unreadable log |

## Rows that need a decision

B-remove (37), B-keep-needs-logging (4) and C (2) are the rows above with those verdicts. Consequences of removal, so the reviewer can weigh them:

- The JSON, diagnose and `urlparse` B-remove rows in probes that already wrap their body in `except Exception` (25-29, 31 `run_query`/`run_engine_query`) turn a hidden default into a recorded ERROR status: strictly more honest, no run aborts.
- The `urlparse` helper rows (`_domain`, `_base_domain`, `_has_real_path`, `_url_domain`, `_match_heuristics`) sit in loops without their own handler; a malformed URL would abort the report step. 0 of 9208 observed.
- `_extract_citation_pdf_url` (16), the stage3 GPU handlers (`_rerank_pool`, `_apply_m9`, `_call_generator`, `stage3_method_run._apply_c3`, `value_eval_probe._apply_c3`) and `13_free_word_probe.run_probe`: removal aborts the run at the first failure instead of continuing with a silent empty result. If the reviewer prefers continuing, the alternative that satisfies the standard is an A-style rewrite that records the failure in the artifact (as `_get_pdf_and_save` and the books/docs probes already do), not a bare `except`.
- `cdp_query_selector` (altcha): removal makes a CDP protocol error abort the trigger attempt instead of falling through to the next locate strategy; `run_trigger_attempt` already keeps its `finally` teardown.
- The four B-keep-needs-logging rows keep their retry; the change would be adding a failure field to the per-query summary (`c3_failed` / `c4_failed`) so the empty ranking is externally distinguishable.


## Phase 4 step 2: approved removals applied (2026-09-24)

Decisions from the orchestrator: remove all 37 B-remove rows, remove the swallow in `00_single_query._print_parsed_results`, rewrite `13_free_word_probe.run_probe` A-style like its books/docs siblings, keep the four retry loops and make a final failure distinguishable from an empty ranking.

What changed, exactly:

- 38 handlers removed (37 B-remove plus the `00_single_query` C row). Each `try` with a single handler was replaced by its body, one indent level shallower; nothing else in those functions moved. Consequences: a JSON, diagnose or `urlparse` failure in the browser probes now surfaces in the probe's existing outer `except Exception`, which records status ERROR with the error text; helpers without such an outer handler (`_domain`, `_base_domain`, `_has_real_path`, `_url_domain`, `_match_heuristics`, `read_counts`, `run_js_dict`, `_repr_unquote`, `_read_last_log_entry`, `_classify_html_body`, `cdp_query_selector`, `_clean_url`) and the run-level GPU and citation-fetch functions (`_rerank_pool`, `_apply_m9`, `_call_generator`, `stage3_method_run._apply_c3`, `value_eval_probe._apply_c3`, `16_search_to_pdf_probe._extract_citation_pdf_url`) now abort the run at the first failure. `import sys` in `_stage3_method_run_v3_gpu.py` became unused with the handler and was removed.
- `13_free_word_probe.py`: `run_probe` now keeps `run_stats` per engine (`total`, `errors`), the report gains a `Run Stats` section (`Errors / Empties` per engine, mean per run over all runs, total URLs), `write_report` and `_build_report` take `run_stats`. The exception is still caught per engine call, printed, and now also counted, exactly as `19_books_probe` and `20_docs_probe` do.
- `ranking_eval/pooling_probe.py`: `_safe_rerank` and `_safe_embed` return `(top, failed)`; `failed` is true only when both attempts raised. The per-query summary (and the JSONL) carries `c3_failed` and `c4_failed` (false for skipped queries), the C3 and C4 headings in a query section read `... API FAILED after retry`, the global summary rows carry ` C3_FAILED` / ` C4_FAILED`, and the report header shows `API failures after retry (C3 / C4): n / m`.
- `ranking_eval/single_query_pool_dump.py`: `_ce_top_scored` and `_embed_top_scored` return `(scored, failed)`; the C3 and C4 headings in Section 3 read `... API FAILED after retry`, and the stderr summary line ends with `C3_failed=... C4_failed=...`.
- No test in `dev/tests` covers these dev scripts; no `src/` file was touched.

Proof:

- AST diff of every touched file against `HEAD`: 55 functions differ, and all 55 are the intended ones (38 unwrapped, plus `13_free_word_probe` `run_probe`/`write_report`/`_build_report`/new `_run_stats`, plus the pooling and single-query-dump functions above); the only top-level change is the removed `import sys`. `py_compile` passes for every file; no function reaches 50 LOC; no comment or docstring reappeared (scan prints nothing); no new unused import.
