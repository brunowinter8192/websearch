...
from _brave_probe_core import (
    ...
    diff_cookie_fingerprints,
)
from _brave_pure_checks import run_pure_function_checks
from _brave_probe_launch import kill_tab, launch_browser, new_tab, resolve_chromium_bundle_path, teardown
from _brave_probe_report import build_report_md
from brave_pydoll_probe import Phase, build_persistence_record, count_live_requests, snapshot_cookie_store
from _brave_probe_query import read_cookie_fingerprints, run_query, verify_environment_reachable
...
async def check_button_challenge_success_path(handle, base_url: str) -> None:
    ...
    check(
        ...
        f"attempted={m.trigger_attempted} found={m.trigger_found_button} mechanism={m.trigger_mechanism}",
    )
    check(
        "a light-DOM button is not reported as being in a shadow root",
        m.button_in_shadow is False, f"got {m.button_in_shadow!r}",
    )
    check(
        "a solved button challenge ending in results is SUCCESS_CHALLENGED",
    ...
    check(
        ...
        m.first_poll_state not in (STATE_RESULTS, STATE_POW_LINK_BLOCK, STATE_BLOCKED),
        f"got {m.first_poll_state}",
    )
    check(
        "timing marks are ordered nav -> button -> fired -> results",
        _marks_are_ordered(m),
        f"nav={m.nav_ms} button={m.button_seen_ms} fired={m.trigger_fired_ms} results={m.results_ms}",
    )
...
async def check_button_challenge_success_shadow_path(handle, base_url: str) -> None:
    ...
    check("a shadow-hosted button is reported as being in a shadow root", m.button_in_shadow is True)
    check(
        "a solved shadow-hosted challenge ending in results is SUCCESS_CHALLENGED",
        m.verdict == VERDICT_SUCCESS_CHALLENGED,
        f"got {m.verdict}",
    )
...
async def check_refused_challenge_is_blocked(handle, base_url: str) -> None:
    ...
    check(
        "the refused run passed through BUTTON_PENDING and BUTTON_VERIFYING first",
        STATE_BUTTON_PENDING in m.state_trace and STATE_BUTTON_VERIFYING in m.state_trace,
        f"got {m.state_trace}",
    )
    check(
...
async def check_pow_link_block_with_no_button(handle, base_url: str) -> None:
    ...
    check(
        ...
        and not m.trigger_attempted,
        f"got {m.verdict}/{m.final_state} attempted={m.trigger_attempted}",
    )
    check(
        "a pow-link block terminates quickly rather than spending its whole budget",
        m.total_ms < FIXTURE_BUDGET_S * 1000 * 0.5,
        f"got {m.total_ms}ms",
    )
